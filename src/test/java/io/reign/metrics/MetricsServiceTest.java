...
public class MetricsServiceTest {
	private static final Logger logger = LoggerFactory
			.getLogger(MetricsServiceTest.class);
	...
	public void testObserver() throws Exception {
		MetricRegistryManager registryManager = getMetricRegistryManager(new RotatingMetricRegistryManager(
				300, TimeUnit.SECONDS));
		Counter observerTestCounter = registryManager.get().counter(
				"observerTestCounter");
			...
			public void updated(MetricsData updated, MetricsData previous) {
				...
				logger.debug(
						...
						updated != null ? updated
								.getCounter("observerTestCounter") : null,
		...
		metricsService.scheduleExport("clusterA", "serviceC", registryManager,
				1, TimeUnit.SECONDS);

		// has not been updated yet
		assertTrue("calledCount should be 0, but is " + calledCount.get(),
				calledCount.get() == 0);

		// wait for update
		synchronized (calledCount) {
			calledCount
					.wait(metricsService.getUpdateIntervalMillis() * 4 + 5000);
		}
		assertTrue("Expected 1, got " + calledCount.get(),
				calledCount.get() == 1);
		...
		assertTrue("calledCount should be >1, but is " + calledCount.get(),
				calledCount.get() > 1);
		assertTrue(
				"latest.observerTestCounter should be "
						+ (previousValue + 1)
						+ ", but is "
						+ latest.get().getCounter("observerTestCounter")
								.getCount(),
	...
	public void testIntervalNodes() throws Exception {
		...
		final MetricRegistryManager registryManager = new RotatingMetricRegistryManager(
				5, TimeUnit.SECONDS);
		...
		metricsService.scheduleExport("clusterA", "serviceD", registryManager,
				1, TimeUnit.SECONDS);
		...
		MetricsData metricsData = metricsService.getServiceMetrics("clusterA",
				"serviceD");

		logger.debug("metricsData={}", JacksonUtil.getObjectMapper()
				.writeValueAsString(metricsData));

		assertTrue("Unexpected value:  " + metricsData.getDataNodeCount(),
				metricsData.getDataNodeCount() == 2);
		assertTrue(
				"Unexpected value:  " + metricsData.getDataNodeInWindowCount(),
				metricsData.getDataNodeInWindowCount() == 1);
		assertTrue("Unexpected value:  "
				+ metricsData.getCounter("c1").getCount(), metricsData
				.getCounter("c1").getCount() == 1);
	}
	...
	public void testExportSelfMetrics() throws Exception {
		MetricRegistryManager registryManager = getMetricRegistryManager(new RotatingMetricRegistryManager(
				300, TimeUnit.SECONDS));
		metricsService.scheduleExport("clusterA", "serviceA", registryManager,
				5, TimeUnit.SECONDS);

		MetricsData metricsData = metricsService.getMyMetrics("clusterA",
				"serviceA");
		...
		assertTrue("Unexpected value:  " + counter1Data.getCount(),
				counter1Data.getCount() == 1L);
		assertTrue("Unexpected value:  " + counter2Data.getCount(),
				counter2Data.getCount() == 2L);
		...
		assertTrue("Unexpected value:  +" + timer1.getMax(),
				Math.floor(timer1.getMax()) - 100 < 5);
		assertTrue("Unexpected value:  +" + timer2.getMax(),
				Math.floor(timer2.getMax()) - 200 < 5);
	...
	public void testExportServiceMetrics() throws Exception {
		...
		metricsService.scheduleExport("clusterA", "serviceB", "node1",
				registryManager1, 1, TimeUnit.SECONDS);

		MetricRegistryManager registryManager2 = getMetricRegistryManager(new StaticMetricRegistryManager());
		metricsService.scheduleExport("clusterA", "serviceB", "node2",
				registryManager2, 1, TimeUnit.SECONDS);

		// get service metrics, but wait for both service nodes to be there
		// before checking values
		MetricsData metricsData = null;
		while ((metricsData = metricsService.getServiceMetrics("clusterA",
				"serviceB")) == null || metricsData.getDataNodeCount() < 2) {
	...
	MetricRegistryManager getMetricRegistryManager(
			MetricRegistryManager registryManager) throws Exception {
		...
		// counters
		Counter counter1 = registryManager.get().counter(
				MetricRegistry.name("counter1"));
		Counter counter2 = registryManager.get().counter(
				MetricRegistry.name("counter2"));
		counter1.inc();
		counter2.inc(2);

		// gauges
		Gauge<Integer> gauge1 = registryManager.get().register(
				MetricRegistry.name("gauge1"), new Gauge<Integer>() {
					@Override
					public Integer getValue() {
						return 1;
					}
				});
		Gauge<Integer> gauge2 = registryManager.get().register(
				MetricRegistry.name("gauge2"), new Gauge<Integer>() {
					@Override
					public Integer getValue() {
						return 2;
					}
		...
		// meters
		Meter meter1 = registryManager.get().meter(
				MetricRegistry.name("meter1"));
		Meter meter2 = registryManager.get().meter(
				MetricRegistry.name("meter2"));
